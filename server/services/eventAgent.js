...
Keep it simple and ensure the JSON is valid. Focus on practical, easy recipes.`;
  ...
  #createSimpleMealPlan(days, people, diet, targetCalories, exclude) {
    ...
    let text = `📅 ${days}-Day ${dietLabel} Meal Plan for ${people} ${people === 1 ? 'person' : 'people'}\n\n`;
    text += `🎯 Target: ${targetCalories} calories per day per person\n`;
    ...
    for (let i = 0; i < days && i < 7; i++) {
      text += `━━━ Day ${i + 1}: ${dayNames[i]} ━━━\n\n`;
      text += `🍽️  BREAKFAST: ${meals.breakfast[i]}\n`;
      text += `   ⏱️  Prep Time: 15-20 minutes\n\n`;
      text += `🍽️  LUNCH: ${meals.lunch[i]}\n`;
      text += `   ⏱️  Prep Time: 20-30 minutes\n\n`;
      text += `🍽️  DINNER: ${meals.dinner[i]}\n`;
      text += `   ⏱️  Prep Time: 30-40 minutes\n\n`;
    }
    
    text += '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    text += '✨ Simple Meal Plan (AI Generation Fallback)\n';
    text += '💡 Tip: Adjust portions and ingredients based on your preferences';
    
    return text;
  }

  #formatMealPlanAsText(mealPlanData, days, people) {
    let text = `📅 ${days}-Day Meal Plan for ${people} ${people === 1 ? 'person' : 'people'}\n\n`;
    
    if (mealPlanData.week) {
      const dayNames = Object.keys(mealPlanData.week);
      dayNames.forEach((day, index) => {
        const dayData = mealPlanData.week[day];
        text += `━━━ Day ${index + 1}: ${day.charAt(0).toUpperCase() + day.slice(1)} ━━━\n\n`;
        
        if (dayData.meals && Array.isArray(dayData.meals)) {
          dayData.meals.forEach(meal => {
            const mealType = meal.type || 'Meal';
            text += `🍽️  ${mealType.toUpperCase()}: ${meal.title}\n`;
            text += `   ⏱️  Prep Time: ${meal.readyInMinutes || 'N/A'} minutes\n`;
            
            if (meal.nutrition) {
              text += `   📊 Nutrition: ${meal.nutrition.calories || 'N/A'} cal`;
            ...
            if (meal.ingredients && meal.ingredients.length > 0) {
              text += `   🛒 Key Ingredients: ${meal.ingredients.slice(0, 5).join(', ')}`;
            ...
            if (meal.instructions) {
              ...
              text += `   📝 ${shortInstructions}${meal.instructions.length > 100 ? '...' : ''}\n`;
            }
    ...
    text += '💡 Tip: Adjust portions based on your needs';
    return text;
  }
}
