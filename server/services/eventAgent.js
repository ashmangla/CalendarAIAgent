 ...
 * @property {Object|null} mealPlanResult Successful meal-plan result (document, nutrients, preferences)
...
- If the external tool fails but fallback plan text is supplied, treat it the same way as formattedText—summarize it and reference it in your tasks.
- If no meal plan data is provided, leave "mealPlan" null and "requiresMealPlanPreferences" false; do not invent meal-plan tasks.`;
  ...
  #shouldGenerateMealPlan(event) {
    ...
    if (!hasPrep) {
      console.log(`[meal-plan-detection] ❌ No "prep" keyword found - skipping meal plan`);
      return false;
    }
    ...
    if (hasMealKeyword) {
      console.log(`[meal-plan-detection] ✅ Meal prep event detected - will attempt meal plan generation`);
    } else {
      console.log(`[meal-plan-detection] ❌ No meal keywords found - skipping meal plan`);
    }
  ...
  async #prepareMealPlanContext({ event, tokens, mealPlanPreferences, shouldAttemptMealPlan }) {
    ...
    if (!shouldAttemptMealPlan) {
      console.log('[meal-plan-context] ❌ Skipping - shouldAttemptMealPlan is false');
      return { mealPlanResult: null, mealPlanError: null, mealPlanFallback: null };
    }

    if (!tokens?.access_token) {
      console.log('[meal-plan-context] ❌ Skipping - No Google tokens available');
      return {
        mealPlanResult: null,
        mealPlanError: new Error('Google authentication not available for meal plan generation.'),
        mealPlanFallback: null
      };
    }

    console.log('[meal-plan-context] ✅ Attempting to generate meal plan...');
    try {
      ...
      console.log('[meal-plan-context] ✅ Meal plan generated successfully');
      return { mealPlanResult, mealPlanError: null, mealPlanFallback: null };
    } catch (error) {
      console.error('[meal-plan-context] ❌ Meal plan generation failed:', error.message);
      ...
      if (fallback) {
        console.log('[meal-plan-context] ✅ LLM fallback meal plan generated');
        return { mealPlanResult: null, mealPlanError: null, mealPlanFallback: fallback };
      }
      console.log('[meal-plan-context] ❌ LLM fallback also failed');
  ...
  async #generateMealPlanFallback(event, preferences = {}) {
    try {
      ...
      const fallbackPrompt = `You are a meal planning expert. Generate a ${days}-day meal plan for ${people} people.
...
Additional user preferences from event description:
${event.description || 'No additional preferences'}
...
Generate practical, realistic meals that match the dietary requirements and calorie targets. Return ONLY valid JSON.`;
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        max_tokens: 2000,
        messages: [
          { role: 'system', content: 'You are a meal planning expert. Return only valid JSON without any markdown formatting or code blocks.' },
      ...
      try {
        ...
        console.error('[meal-plan-fallback] Response was:', responseText.substring(0, 200));
        // Return a user-friendly error message instead of raw JSON
        return `Meal Plan Generation\n\nA meal plan was generated but couldn't be formatted properly. Please try again or check with your administrator.\n\nError: ${parseError.message}`;
      }
    } catch (fallbackError) {
  ...
  #formatMealPlanAsText(mealPlanData, days, people) {
    let text = `Meal Plan for ${days} days (${people} people)\n\n`;
      ...
      dayNames.forEach(day => {
        ...
        if (dayData.meals && Array.isArray(dayData.meals)) {
          dayData.meals.forEach(meal => {
            text += `  - ${meal.title} (${meal.readyInMinutes || 'N/A'} min, ${meal.nutrition?.calories || 'N/A'} cal)\n`;
            if (meal.ingredients && meal.ingredients.length > 0) {
              text += `    Ingredients: ${meal.ingredients.slice(0, 3).join(', ')}${meal.ingredients.length > 3 ? '...' : ''}\n`;
            }
          });
        }
        text += '\n';
